...
class EigthPuzzleProblema(Problema):
    ...
    #def h(self, nodo):
    #     l = sum ([1 for i in range(8) if nodo.estado[i] != OBJETIVO[i]])
    #     return l
...
def test_busqueda_recursiva_primero_mejor():
    return busqueda_recursiva_primero_mejor(eight_puzzle)
...
def main():
    ...
    #nodo_solucion = test_busqueda_recursiva_primero_mejor()
    ...
    print("---%s segundos---"%(tiempo_ejecucion))
    print("Numero de iteraciones:", d + 1)
    print("Numero total de nodos A*:", N)
    print("Factor de ramificacion es: ", B)
